...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // username is picked up by the DB trigger that populates public.users
        data: { username },
      },
    });
    ...
    // Auto-sign-in after signup (email confirmation disabled for local dev)
    router.push("/");
    router.refresh();
  };
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", alignItems: "center", justifyContent: "center", background: "#212121" }}>
      <div style={{ width: 360, padding: 32, borderRadius: 16, background: "#2a2a2a", border: "1px solid #3f3f3f" }}>
        ...
        <p style={{ color: "#6b7280", fontSize: 13, textAlign: "center", marginBottom: 28 }}>Create your account</p>

        <form onSubmit={handleSignup} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            <label style={{ display: "block", color: "#8e8ea0", fontSize: 12, marginBottom: 6, fontWeight: 500 }}>Username</label>
            <input
        ...
        <p style={{ color: "#4b5563", fontSize: 13, textAlign: "center", marginTop: 20 }}>
          ...
          <Link href="/auth/login" style={{ color: "#8b5cf6", textDecoration: "none" }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
